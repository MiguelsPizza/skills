...
I think that is the main thing newer AI users miss. The problem is not usually that the code is obviously terrible. The problem is that it quietly creates parallel truths.
...
But now you have two validators for one concept. One trims `couponCode`; the other does not. One reuses `userIdSchema`; the other weakens it to `z.string()`. One is probably where future people will look. The other is probably where the last agent happened to be standing.
...
The clean version is to update the real owner:
...
That looks boring because it is boring. The schema changed, the owner stayed the owner, and callers just pass the newer shape.
...
That is how one real owner turns into three local ones:
...
None of them look outrageous in isolation. Together they make it much less obvious where a future change is supposed to go.
...
It is often tidy, typed, tested, and still wrong about where reality lives.
