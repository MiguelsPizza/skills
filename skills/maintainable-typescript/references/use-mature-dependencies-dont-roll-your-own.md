...
**Rule:** Do not rebuild commodity infrastructure in application code. Use mature, well-documented, widely adopted dependencies for solved problems. If no such dependency is clearly known and approved, keep the implementation local and minimal.
...
The opposite failure also happens. Agents sometimes install random packages with thin ecosystems, weak docs, and unclear maintenance because the name sounds right. That trades one long-term problem for another.
...
4. If a mature dependency is not clearly known, do not guess with an obscure package
5. In that case, write the smallest local implementation that solves the immediate need without pretending to be a reusable framework
...
import type { InstallationId } from '@repo/shared-types/installation-id';
import { githubClient } from '@/lib/github-client';

const MAX_GITHUB_FETCH_ATTEMPTS = 3;
